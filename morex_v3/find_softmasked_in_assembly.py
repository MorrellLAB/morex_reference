...
are softmasked (lowercase) regions only and prints output to
stdout in BED format. Note: this script doesn't include hardmasked
regions (N's), see script find_Ns_in_assembly.py instead.

Usage: ./find_softmasked_in_assembly.py [fasta_fp] > out_file.bed
...
with open(fasta_fp, 'r') as handle:
    for record in SeqIO.parse(handle, "fasta"):
        for match in re.finditer('[a-z]+', str(record.seq)):
